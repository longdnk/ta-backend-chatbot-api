...
from graph.message_state import message
    ...
    async def generate():
        ...
        async for chunk in app_graph.astream(
            ...
            if "grade_documents" in chunk:
                ...
                for index, document_object in enumerate(chunk["grade_documents"]["documents"]):
                    ...
                    yield f"""<details open style="
                                    ...
                                    color: {color};
                                ">
                                <summary>Document {index+1} -- <B>{title}</B></summary>
                                <p style="padding-left: 20px;">{content}</p>
                        </details>"""
                    await asyncio.sleep(0.5)
                yield f"""</blockquote>"""

            if "transform_query" in chunk:
                yield f"""<blockquote style="margin-bottom: 24px;padding: 10px;border: 2px solid;border-color: #4229fb;border-radius: 6px;">
                        <strong>
                            Transform query: <span style="color: blue;">{chunk['transform_query']['question']}</span>
                        </strong>
                    </blockquote>"""
                await asyncio.sleep(0.5)
            ...
            if "generate" in chunk:
                ...
                yield f"""<blockquote style="margin-bottom: 24px;padding: 10px;border: 2px solid;border-color: #4229fb;border-radius: 6px;">
                        <strong>
                            Generate Response <span style="color: green;">{message['generation_result_message']}</span></strong>
